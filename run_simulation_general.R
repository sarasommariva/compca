...
eigenval_y=c(10, 9, 1, 1,   0.5, 0.4, 0.1, 0.01)
...
all_distributions = c('Gaussian', 'Uniform', 'Mult4', 'Mult8', 'Mult40')
...
for (ii in 1:length(all_distributions)){
  if (all_distributions[ii] == 'Gaussian'){
    distribution = 'Gaussian'; df = NULL
  }else if(all_distributions[ii] == 'Uniform'){
    distribution = 'Uniform'; df = NULL
  }else if(all_distributions[ii] == 'Mult4'){
    distribution = 'Mult'; df = 4
  }else if(all_distributions[ii] == 'Mult8'){
    distribution = 'Mult'; df = 8
  }else if(all_distributions[ii] == 'Mult40'){
    distribution = 'Mult'; df = 40
  }

  cat('---------- Distribution: ', distribution, 
      ' df = ', if (is.null(df)) "N/A" else df, ' \n')
  ...
  # Step 5. Randomly generate data
        ...
        # Test on the ilr transformation
        Y_transf_2 <- robCompositions::bpc(Y_comp)$Coordinates
        Z_transf_2 <- robCompositions::bpc(Z_comp)$Coordinates
        print('Test on ilr transformation')
        print(max(abs(Y_transf-(-Y_transf_2))))
        print(max(abs(Z_transf-(-Z_transf_2))))
        
        Y_transf_3 = matrix(data=NA, nrow=n_y, ncol=D-Q1-1)
        Z_transf_3 = matrix(data=NA, nrow=n_z, ncol=D-Q2-1)
        for (id in 1:n_y){
          Y_transf_3[id,] <- as.numeric(clr(Y_comp[id,])) %*% V_ilr_y
        }
        for (id in 1:n_z){
          Z_transf_3[id,] <- as.numeric(clr(Z_comp[id,])) %*% V_ilr_z 
        }
        
        print('Test on ilr transformation via clr')
        print(max(abs(Y_transf-(Y_transf_3))))
        print(max(abs(Z_transf-(Z_transf_3))))
        ...
        #aux2 = compute_pvalue_boot_single(K_H0, num_boot, temp_stat_value, 
        #                                  eigen_cov_y, eigen_cov_z, Y_transf, Z_transf)
        #print('Bootstrap - single')
        #print(aux2$pvalue)
  ...
  # Step ??. Save result
  result = list('param' = list('M'=M, 'Q1'=Q1, 'Q2'=Q2, 'K_H0'=K_H0, 'K_sim'=K_sim,
  ...
  save(result, file=file.path(folder_res, 
                      paste0('res_K_', K_sim, '_distr_', distribution, '_df_', df,'_boot_', num_boot,'_general.Rdata')))
  
  rm(result, results_list, results_list_name)
}